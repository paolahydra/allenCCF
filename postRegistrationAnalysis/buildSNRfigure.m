...
[M,c] = contour(double(isROI_F));

    

% do you need two axis to overlay the image?
figure, hold on %remove
s = scatterhist(Y,Z,'Group',brain,'Kernel','on');
axes(s(1))
axis image
s(1).YDir = 'reverse';
s(3).XDir = 'reverse'; % need to revert the histogram too!!



% select same colormnap as before (in S)
% remove the axes
% put the contour
% make figure white
...
%% can make pretty injection-site figure? %this still needs a lot of work
...
% %make a mock registration and retrieve it
...
figure;
axATL = axes;
[coords, coordsReg, h] = sliceOutlineWithRegionVec(avSlice, VIIN, [1,0,0], );
