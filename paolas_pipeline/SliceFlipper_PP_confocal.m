function SliceFlipper_PP_confocal(slice_figure, folder_processed_images, reference_size, gain)
...
for f = 1 : ud.total_num_files
    ud.processed_transf_names{f} = [ud.processed_image_names{f}(1:end-4) '_transf.mat'];
end
...
fprintf(1, 'scroll: rotate slice - DISABLED \n');
fprintf(1, 's: sharpen - DISABLED\n');
fprintf(1, 'g: toggle grid \n');
fprintf(1, 'c: crop slice further - DISABLED \n');
fprintf(1, 'f: flip horizontally \n');
fprintf(1, 'w: switch order (move image forward) - DISABLED\n');
fprintf(1, 'r: reset to original \n');
fprintf(1, 'n: go to slice num... \n');
fprintf(1, 'i: increase gain... \n');
fprintf(1, 'd: decrease gain... \n');
fprintf(1, 'v: change channel mode view - DISABLED\n'); % under construction
...
%         imwrite(ud.current_slice_image, fullfile(folder_preprocessed_images, ud.processed_image_name))
    ...
    case 'c' % do not allow cropping for confocal images - I would have to save the transformation
...
%         
    case 'w' % switch order
        ...
        % do not allow for confocal images - unless needed. Would need to bring
        % the transformation files together
...
%     ud.current_slice_image = imread(fullfile(folder_processed_images, ud.processed_image_name));
    ...
    if nChannels>1
        ...
        A = imread(fname, 'tif'); %usually RGB images
    end
